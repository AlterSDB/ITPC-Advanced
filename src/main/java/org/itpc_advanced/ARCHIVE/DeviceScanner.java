...
import org.itpc_advanced.controller.FXMLMainController;
import org.itpc_advanced.model.ByteSequence;
...
public class DeviceScanner {
	...
	private static ObservableList<DataFileOld> files;
	...
	//	files = FXCollections.observableArrayList(new DataFile(1), new DataFile(2), new DataFile(3), new DataFile(4), new DataFile(5), new DataFile(1), new DataFile(2), new DataFile(3));
		...
		private void readFiles() throws InterruptedException, SerialPortException {
			...
			for(int i = 1; i <= 8; i++) {
				...
				files.add(new DataFileOld(readData(request)));
				request = ByteSequence.nextFile(request);
			}
			System.out.println("Успешно прочитано " + DataFileOld.getCount());

			for(DataFileOld df : files) {
		...
		public ObservableList<DataFileOld> getDataFiles() {
			return files;
		};
